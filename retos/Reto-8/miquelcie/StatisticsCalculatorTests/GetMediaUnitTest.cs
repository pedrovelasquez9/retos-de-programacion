...
{
    public class GetMediaUnitTests
    {
        /*
         Enunciado
 ...
 Si llamo a la función getModa(numberList), el resultado esperado, según los cálculos explicados, debe ser [1,2,3,4,5] dado que ningún valor se repite más de una vez.
        ...
        [Test]
        public void GetMedia_WhenCalledNullValue_ReturnException()
        {
            Assert.Throws<ArgumentNullException>(() => StatiticsCaluculator.GetMedia(null));
        }

        [Test]
        public void GetMedia_WhenCalledEmptyList_ReturnException()
        {
            List<int> numbers = new List<int>();

            Assert.Throws<ArgumentOutOfRangeException>(() => StatiticsCaluculator.GetMedia(numbers));
        }

        [TestCase]
        public void GetMedia_WhenCalled_ReturnCorrectValue()
        {
            List<int> numbers = new List<int>() { 1 };

            int expectValue = 1;
            Assert.That(StatiticsCaluculator.GetMedia(numbers), Is.EqualTo(expectValue));
            ...
            numbers = new List<int>() { 5, 8, 12, 15 };
            expectValue = 10;
            Assert.That(StatiticsCaluculator.GetMedia(numbers), Is.EqualTo(expectValue));
